...
cancelled" appears _under_ "Uploading…", and the Upload button stays
...
are _meaningful_? Idle, uploading, failed, done, cancelled — five. The
...
The upload is a **state machine**: it is always in exactly _one_ state,
and events (start, fail, succeed, cancel, retry) are _transitions_. Model
it as one value with five possible states and both tickets become
unrepresentable rather than unhandled — a transition _replaces_ the
...
- A successful retry shows "complete" and no failure banner (Ticket A
  test).
- **At most one** of spinner / error / done is on screen at any point in
  the flow — including the moment a retry starts, which is "in progress",
  not "still failed".
- Cancel returns the panel to an actionable state: no spinner, Upload
  re-enabled (Ticket B test) — and a fresh upload after a cancel runs to
  completion.
