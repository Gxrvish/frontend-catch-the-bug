...
closes _and the lightbox opens_, as if you'd clicked the card itself. Same
...
Clicking anywhere _inside_ the dialog that isn't a button — the title, the
...
- **Ticket A (synthetic events):** React does not propagate events along
  ...
  moves where the DOM nodes _land_, but the dialog is still a React child
  ...
  made it unnecessary.
- **Ticket B (native events):** the outside-click handler is a _native_
  `document` listener, and it decides inside/outside with
  `containerRef.current.contains(event.target)` — a **DOM** containment
  check against the _card_. In the DOM, the portal's nodes really are
...
- Dismissing (or confirming) the dialog must not trigger the card's click
  — no lightbox (Ticket A test). Confirming travels the same path Cancel
  does, so **Delete** is held to the same rule.
- Clicks inside the dialog must not dismiss it (Ticket B test) — and must
  not open the lightbox either. Outside really is outside: a click on the
  page still dismisses.
