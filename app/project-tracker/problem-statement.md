...
Rename _Fix login redirect_ in My tasks — the sprint board keeps showing
...
Delete _Write release notes_ from the sprint board — it stays in My tasks
...
never _if_ they diverge but _when_.
...
_references_ (ids) it resolves at render time.

## Requirements for the Fix

- A rename in My tasks must show on the sprint board (Ticket A test) —
  for an in-progress task as well as a to-do one. The other column is the
  same rule, not a second one.
- A delete on the board must remove the task from My tasks (Ticket B
  test), and both panels' counts must drop together.
- A rename followed by a delete of the same task stays consistent: the
  rename reaches the board's delete control, and the delete reaches the
  task panel.
